...
public class UserController {
	...
	public ResponseEntity<?> login(@Valid @RequestBody LoginForm form, Errors errors) throws Exception {
		Result result = new Result();
		if (errors.hasErrors()) {
			for(ObjectError err: errors.getAllErrors()) {
				result.getMessages().add(err.getDefaultMessage());
		...
		User output = userService.login(form.getEmail(), form.getPassword());
		result.setPayload(output);
		return  ResponseEntity.ok(result);
	}
